...
const allProvidersData = [
    ...
    { name: "SoFi", rating: 4.7, reviews: "15420", aprRange: "8.99% - 29.49%", amountRange: "$5,000 - $100,000", time: "Next Business Day", tags: ['No fees', 'Membership perks', 'Featured'], minApr: 8.99, maxApr: 29.49, minAmount: 5000, maxAmount: 100000, minCreditScore: 680, speed: "Fast", loanTypes: ["Personal", "Student Refinance"], hasFees: false, loanTerm: "2-7 years", logoType: "initial", logoContent: "S", logoBg: "bg-teal-500", isTopTen: false, originationFee: "0%", lateFee: "Not specified", prepaymentPenalty: false, website: 'https://www.sofi.com/' },
    ...
    { name: "Upstart Personal Loans", rating: 4.4, reviews: "38000", aprRange: "6.5% - 35.99%", amountRange: "$1,000 - $50,000", time: "Next Business Day", tags: ['AI underwriting', 'Fast approval', 'Featured'], minApr: 6.5, maxApr: 35.99, minAmount: 1000, maxAmount: 50000, minCreditScore: 580, speed: "Fast", loanTypes: ["Personal", "Auto Refinance"], hasFees: true, loanTerm: "3-5 years", logoType: "initial", logoContent: "U", logoBg: "bg-gray-700", isTopTen: false, originationFee: "0% - 12%", lateFee: "$15", prepaymentPenalty: false, website: 'https://www.upstart.com/' },
    ...
    { name: "LightStream Auto Loans", rating: 4.7, reviews: "15000", aprRange: "6.24% - 25.49%", amountRange: "$5,000 - $100,000", time: "Same Day", tags: ['Excellent rates', 'Same-day funding', 'No fees'], minApr: 6.24, maxApr: 25.49, minAmount: 5000, maxAmount: 100000, minCreditScore: 680, speed: "Fast", loanTypes: ["Personal", "Auto"], hasFees: false, loanTerm: "2-12 years", logoType: "initial", logoContent: "L", logoBg: "bg-teal-400", isTopTen: false, originationFee: "0%", lateFee: "Not specified", prepaymentPenalty: false, website: 'https://www.lightstream.com/' },
...
const heroPills = ['All Types', 'Personal Loan', 'Debt Consolidation', 'Auto Loan', 'Home Improvement'];
...
// --- MAIN COMPONENT ---
const AllProviders: React.FC<{ onCompare: (providers: Provider[]) => void; }> = ({ onCompare }) => {
    ...
    const filteredProviders = useMemo(() => {
        return allProvidersData.filter(provider => {
            const loanTypeMatch = loanType === 'All Types' || provider.loanTypes.some(lt => loanType.toLowerCase().includes(lt.toLowerCase()));
            const searchMatch = !searchQuery || provider.name.toLowerCase().includes(searchQuery.toLowerCase());
            const amountMatch = loanAmount >= provider.minAmount && loanAmount <= provider.maxAmount;
            return loanTypeMatch && searchMatch && amountMatch;
        });
